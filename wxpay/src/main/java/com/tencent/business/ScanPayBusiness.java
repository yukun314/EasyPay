...
//    private static Log log = new Log(LoggerFactory.getLogger(ScanPayBusiness.class));
    ...
    public void run(ScanPayReqData scanPayReqData, ResultListener resultListener) throws Exception {
        ...
        if (Configure.isUseThreadToDoReport()) {
            ...
            System.out.println("pay+report总耗时（异步方式上报）：" + (timeAfterReport - costTimeStart) + "ms");
        } else {
            ReportService.request(reportReqData);
            timeAfterReport = System.currentTimeMillis();
            System.out.println("pay+report总耗时（同步方式上报）：" + (timeAfterReport - costTimeStart) + "ms");
        }
            ...
            if (scanPayResData.getResult_code().equals("SUCCESS")) {
                ...
                if (errorCode.equals("AUTHCODEEXPIRE") || errorCode.equals("AUTH_CODE_INVALID") || errorCode.equals("NOTENOUGH")) {
                    ...
                    if (doPayQueryLoop(payQueryLoopInvokedCount, outTradeNo)) {
                        ...
                        System.out.println("【需要用户输入密码、在一定时间内没有查询到支付成功、走撤销流程】");
                        doReverseLoop(outTradeNo);
                    ...
                    if (doPayQueryLoop(payQueryLoopInvokedCount, outTradeNo)) {
                        ...
                        System.out.println("【支付扣款未知失败、在一定时间内没有查询到支付成功、走撤销流程】");
                        doReverseLoop(outTradeNo);
                        resultListener.onFail(scanPayResData);
                    }
                }
    ...
    private boolean doOnePayQuery(String outTradeNo) throws Exception {
        ...
        System.out.println("支付订单查询API返回的数据如下：");
        System.out.println(payQueryServiceResponseString);
        ...
        if (scanPayQueryResData == null || scanPayQueryResData.getReturn_code() == null) {
            System.out.println("支付订单查询请求逻辑错误，请仔细检测传过去的每一个参数是否合法");
            return false;
        }

        if (scanPayQueryResData.getReturn_code().equals("FAIL")) {
            //注意：一般这里返回FAIL是出现系统级参数错误，请检测Post给API的数据是否规范合法
            System.out.println("支付订单查询API系统返回失败，失败信息为：" + scanPayQueryResData.getReturn_msg());
            return false;
        } else {
            if (scanPayQueryResData.getResult_code().equals("SUCCESS")) {//业务层成功
                if (scanPayQueryResData.getTrade_state().equals("SUCCESS")) {
                    ...
                    //支付不成功
                    System.out.println("查询到订单支付不成功");
                    return false;
                }
            } else {
                System.out.println("查询出错，错误码：" + scanPayQueryResData.getErr_code() + "     错误信息：" + scanPayQueryResData.getErr_code_des());
                return false;
            }
        }
    }
    ...
    private boolean doOneReverse(String outTradeNo) throws Exception {
        ...
        System.out.println("撤销API返回的数据如下：");
        System.out.println(reverseResponseString);
        //将从API返回的XML数据映射到Java对象
        ...
        if (reverseResData == null) {
            System.out.println("支付订单撤销请求逻辑错误，请仔细检测传过去的每一个参数是否合法");
            return false;
        }
        if (reverseResData.getReturn_code().equals("FAIL")) {
            //注意：一般这里返回FAIL是出现系统级参数错误，请检测Post给API的数据是否规范合法
            System.out.println("支付订单撤销API系统返回失败，失败信息为：" + reverseResData.getReturn_msg());
            return false;
        } else {
            if (reverseResData.getResult_code().equals("FAIL")) {
                System.out.println("撤销出错，错误码：" + reverseResData.getErr_code() + "     错误信息：" + reverseResData.getErr_code_des());
